...
func generateModule(name string, minimal bool) error {
	...
	// Generate handler.go (skip for quick mode)
	if handlerTempl != "" {
	...
	// Generate additional files only for full mode (not minimal, not quick)
	if !minimal && !quickMode {
	...
	if quickMode {
		readmeTempl = quickReadmeTemplate
	} else if minimal {
